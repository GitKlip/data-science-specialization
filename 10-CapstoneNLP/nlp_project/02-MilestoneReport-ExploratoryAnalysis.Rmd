...
if(exists(cache_data_save_file)){ 
    ...
    source('./src/02-GetAndCleanData.R') 
}
...
```{r download, code=readLines('./src/01-DownloadData.R'), eval=F }
```

###Extract and Process Data (Sample, Tokenize, Calculate Basic Stats)
```{r process, code=readLines('./src/02-GetAndCleanData.R'), eval=F}
source()
```
...
###Script ‘utils.R’
```{r src.utils, code=readLines('./src/utils.R'), eval=F, echo=T, tidy.opts=list(comment=FALSE)}
```
