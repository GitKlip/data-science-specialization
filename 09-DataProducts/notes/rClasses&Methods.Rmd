...
- Basic Idea: want to Represent new data types that R dosn't support by default 
...
- _class_ is a description of an thing. A class can be defined using `setClass()` in the *methods* package.
- _object_ is an instance of a class. Objects can be created using `new()`. 
- _method_ is a function that only operates on a certain class of objects.
- _method_ is the implementation of a generic function for an object of a particular class.
- _generic function_ is an R function which dispatches methods. A generic function typically encapsulates a “generic” concept (e.g. `plot`, `mean`, `predict`, ...). The generic function does not actually do any computation.

---

## Things to look up

- The help files for the ‘methods’ package are extensive — they are the primary documentation
...
```{r, collapse=TRUE}
#The `mean` and `print` functions are generic  S3
mean
print
...
## Examining Code for Methods
  ...
  * If you want to see the code:
    + For S3 call `getS3method(<generic>, <class>)`
    + For S4 call `getMethod(<generic>, <signature>)`
...
## S4 Class/Method: Polygon Method
